...
public class PlayerMovement : MonoBehaviour
{
    ...
    bool isRorating = false;

    public GameObject finalGolem;
    ...
    void OnAnimatorMove()
    {
        ...
        circle.transform.position = transform.position + transform.up*0.5f - transform.forward * 0.2f;
        circle.transform.rotation = Quaternion.Euler(0, transform.rotation.eulerAngles.y, 0);

        magicEffect.transform.position = transform.position + transform.up * 0.5f + transform.forward*4f;
    }
        ...
        public void playMagic()
        {
            ...
            m_Animator.Play("skill3");
            magicEffect.SetActive(true);
            AudioSource.PlayClipAtPoint(_magicSound, transform.position);
            m_Animator.SetBool("IsWalking", false);
        }
